...
import com.cp.s2si.filter.JWTMicroService;
...
public class JWTMicroServiceTests {
	...
	@Spy @InjectMocks
	private JWTMicroService jwtMicroService = new JWTMicroService();
	...
	public void chainIfAlreadyAuthenticated() throws Exception {
		...
		// Executing
		jwtMicroService.doFilter(httpServletRequest, httpServletResponse, filterChain);
	...
	public void chainAndConsumeWithBearerAuthorizationHeader() throws Exception {
		// Mocking
		given(requestAttributes.getAttribute("username", RequestAttributes.SCOPE_REQUEST)).willReturn(null);
		given(httpServletRequest.getHeader("Authorization")).willReturn("MS-Bearer ABC.MNO.XYZ");
		given(jwtService.consumeMicroServiceJWT("ABC.MNO.XYZ")).willReturn("TEST-USER");
	...
	public void chainAndDonotAuthorizeOnException() throws Exception {
		...
		given(requestAttributes.getAttribute("username", RequestAttributes.SCOPE_REQUEST)).willReturn(null);
		given(httpServletRequest.getHeader("Authorization")).willReturn("MS-Bearer ABC.MNO.XYZ");
		given(jwtService.consumeMicroServiceJWT("ABC.MNO.XYZ")).willThrow(new JWTNotATokenException(), new JWTBadEncryptionEception(), new JWTNoPayloadException(), new JWTBadSignException(), new JWTAudienceException(), new JWTExpiredException());
