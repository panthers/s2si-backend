...
/**
 * Copied from class org.hibernate.type.AbstractStandardBasicType
 * 
 * Define fallback sql type incase of non nationalized sql types.
 *
...
public abstract class AbstractStandardBasicType<T>
	...
	private static final long serialVersionUID = 3063085852344476734L;
	...
	private SqlTypeDescriptor fallbackSqlTypeDescriptor;
	private JavaTypeDescriptor<T> javaTypeDescriptor;
	// sqlTypes need always to be in sync with sqlTypeDescriptor
	private int[] sqlTypes;
	private int[] fallbackSqlTypes;
	
	// FIXME BAD static FIX for multiple instances of this type
	private static boolean isFallbackChecked = false; //Should be same irrespective of instance 
	private static boolean isFallbackRequired = false; //Should be same irrespective of instance

	public AbstractStandardBasicType(SqlTypeDescriptor sqlTypeDescriptor, SqlTypeDescriptor fallbackSqlTypeDescriptor, JavaTypeDescriptor<T> javaTypeDescriptor) {
		this.sqlTypeDescriptor = sqlTypeDescriptor;
		this.fallbackSqlTypeDescriptor = fallbackSqlTypeDescriptor;
		this.sqlTypes = new int[] { sqlTypeDescriptor.getSqlType() };
		this.fallbackSqlTypes = new int[] { fallbackSqlTypeDescriptor.getSqlType() };
	...
	public boolean shouldFallback(Mapping mapping) {
		if(mapping instanceof Metadata) {
			Metadata metadata = (Metadata) mapping;
	...
	public final SqlTypeDescriptor getSqlTypeDescriptor() {
		if(isFallbackRequired) {
			return fallbackSqlTypeDescriptor;
		}
	...
	public final void setFallbackSqlTypeDescriptor( SqlTypeDescriptor fallbackSqlTypeDescriptor ) {
		this.fallbackSqlTypeDescriptor = fallbackSqlTypeDescriptor;
		this.fallbackSqlTypes = new int[] { fallbackSqlTypeDescriptor.getSqlType() };
	}
	...
	public final int[] sqlTypes(Mapping mapping) throws MappingException {
		if(!isFallbackChecked) {
			//Check on dialect
			isFallbackRequired = shouldFallback(mapping);
			//Dialect checked
			isFallbackChecked = true;
		}
		if(isFallbackRequired) {
			return fallbackSqlTypes;
		}
		return sqlTypes;
	}
