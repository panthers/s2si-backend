...
@Component
@Priority(value = 20)
public class JWTMicroService extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (username == null) {
			...
			if (token != null && token.startsWith("MS-Bearer ")) {
				try {
					String user = jwtService.consumeMicroServiceJWT(token.replace("MS-Bearer ", ""));
